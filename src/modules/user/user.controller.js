...
export const authUser = asyncHandler(async (req, res) => {
  ...
  const isNew = !user;
  if (!user) { user = await userModel.create({ email }); await sendEmail(email, 'Welcome to Cartify! 🎉', '<h2>Welcome!</h2><p>Your Cartify account has been created.</p>'); }
  const token = issueToken(res, user);
  const sessionId = req.cookies?.sessionId;
  if (sessionId) { await Promise.all([mergeCart(sessionId, user._id), mergeWishlist(sessionId, user._id)]); clearSessionCookie(res); }
